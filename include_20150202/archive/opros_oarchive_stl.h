...
template <>
inline void save_this<bool>(opros::archive::oarchive &ar, std::vector<bool> &t)
{
	int size = t.size();		// 362ms
	...
	{
		bool x = t[i];
		ar << OPROS_SERIALIZATION_MAKE_NVP( value, x);
	}
}
...
void save_this(opros::archive::oarchive &ar, std::vector<T> &t)
{
	...
	for (i=0;i<size;i++)
	{
		ar << OPROS_SERIALIZATION_MAKE_NVP( value, (T &)t[i]);
	}
}
