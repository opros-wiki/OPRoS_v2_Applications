...
#ifndef EVENT_OPROS
#define EVENT_OPROS 0
#endif

template<typename dataType,int ModType=EVENT_OPROS>
class OutputEventPort{
};

template<typename dataType>
class OutputEventPort<dataType, EVENT_OPROS>: public OutputEventPortInterface {
public:
	OutputEventPort() {
	}
	...
	{
		EventData<dataType> somedata;
		somedata.setContentData(data);

		return push_event(&somedata);
	}
